<template>
  <div
    class="w-full min-h-screen h-screen bg-white overflow-hidden grid grid-cols-1 lg:grid-cols-2"
  >
    <!-- Left Hero Banner (Full Height) -->
    <div
      class="relative bg-black flex flex-col justify-end p-8 sm:p-12 lg:p-16 overflow-hidden h-full min-h-[40vh] lg:min-h-screen"
    >
      <!-- Background Image -->
      <img
        src="/img/background/bg-auth.webp"
        alt="Background Auth"
      ...
      <div class="relative z-10 space-y-5 text-white max-w-lg font-geist">
        ...
        <NuxtLink to="/" class="inline-flex items-center mb-1">
          <img src="/assets/icons/herms_logo.svg.svg" alt="Hermes Logo" class="h-8 sm:h-9 w-auto" />
        </NuxtLink>
    ...
    <div
      class="relative bg-white flex flex-col justify-center items-center p-6 sm:p-12 lg:p-16 overflow-y-auto h-full min-h-[60vh] lg:min-h-screen"
    >
      <!-- Subtle Grid Line Background Pattern -->
      <div
        class="absolute inset-0 pointer-events-none opacity-40 bg-[linear-gradient(to_right,#e5e7eb_1px,transparent_1px),linear-gradient(to_bottom,#e5e7eb_1px,transparent_1px)] bg-[size:48px_48px]"
      ></div>

      <!-- Auth Card Content -->
      <div
        class="relative z-10 w-full max-w-3xl flex flex-col items-center text-center my-auto font-geist"
      >
        <!-- Tag Badge -->
        <div class="mb-4">
          <span
            class="inline-flex items-center px-3.5 py-1 rounded-full text-s font-medium bg-[#FFF7ED] text-[#E75A0F]"
          >
            Selamat Datang!
          </span>
        </div>
        ...
        <!-- Main Title -->
        <h1
          class="font-urbanist text-2xl sm:text-3xl lg:text-[40px] font-semibold text-gray-900 tracking-tight mb-3"
        >
        ...
        <p class="font-geist text-[20px] text-[#0A0A0A] leading-relaxed mb-16 max-w-3xl">
          ...
          jadi bagian dari perubahan positif di sekitar kita!
        </p>
        ...
        <div class="w-full space-y-6 flex flex-col items-center">
          <!-- Agreement Checkbox -->
          <label
            class="flex items-start gap-3 text-left text-[16px] text-gray-700 w-full max-w-[750px] pl-6 sm:pl-12 cursor-pointer select-none"
          >
            <input
              type="checkbox"
              v-model="agreed"
            ...
            <span class="leading-snug">
              ...
              <button
                type="button"
                class="font-bold text-gray-900 hover:text-[#E75A0F]"
          ...
          >
            ...
            <svg class="w-6 h-6 sm:w-6 sm:h-6 shrink-0" viewBox="0 0 24 24">
              <path
                :fill="agreed ? 'currentColor' : '#9E9E9E'"
