  ...
  >
    <div v-if="report" class="space-y-6">
      ...
      <div class="flex flex-wrap gap-2 text-xs">
        Saya me
        <span
          class="bg-gray-100 text-gray-700 px-3 py-1 rounded-lg font-medium flex items-center gap-1.5"
        >
          <svg
      ...
      <!-- Description Content -->
      <div
        class="bg-gray-50 p-4 rounded-xl text-sm text-gray-700 leading-relaxed border border-gray-100"
      >
      ...
      <div v-if="report.images && report.images.length > 0" class="space-y-2">
        ...
        <div class="flex flex-wrap gap-3">
          <template v-for="(img, idx) in report.images" :key="idx">
            ...
            >
              <div
                class="flex h-9 w-9 items-center justify-center rounded-lg bg-red-100 text-red-600 font-bold text-xs"
              >
                PDF
              </div>
              <div>
                <span class="block text-xs font-semibold text-gray-800"
                  >Lihat / Unduh Dokumen PDF</span
                >
      ...
      <div class="space-y-4 pt-4 border-t border-gray-100">
        <h4 class="text-sm font-bold text-gray-900 flex items-center gap-2">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="h-5 w-5 text-brand-orange"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
        ...
        <div v-if="report.comments && report.comments.length > 0" class="space-y-3">
          ...
          >
            ...
            <div class="flex-1 text-xs sm:text-sm">
              ...
              <p class="text-xs text-brand-orange font-semibold mb-1" v-if="comment.role">
                {{ comment.role }}
              </p>
        ...
        <div
          v-else
          class="p-4 rounded-xl bg-gray-50 text-center text-xs text-gray-500 border border-gray-100"
        >
